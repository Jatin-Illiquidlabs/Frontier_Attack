...
namespace JoshH.UI
{
    ...
    public class UIGradient : Coffee.UIExtensions.BaseMeshEffect
    {
        ...
        [SerializeField] RectTransform overrideRectTransform;
        ...
        public override void ModifyMesh(VertexHelper vh)
        {
            if (this.enabled)
            {
                ...
                bool overrideRectTransformIsPresent = overrideRectTransform != null;
                for (int i = 0; i < vh.currentVertCount; i++)
                {
                    vh.PopulateUIVertex(ref vert, i);

                    Vector2 vertPosition = vert.position;
                    Rect rect;
                    if (overrideRectTransformIsPresent) {
                        vertPosition = rectTransform.TransformPoint(vertPosition);
                        vertPosition = overrideRectTransform.InverseTransformPoint(vertPosition);

                        rect = overrideRectTransform.rect;
                    } else {
                        rect = rectTransform.rect;
                    }

#if UNITY_2018_1_OR_NEWER
                    Vector2 normalizedPosition = (vertPosition - rect.min) / (rect.max - rect.min);
#else
                    Vector2 size = rectTransform.rect.max - rectTransform.rect.min;
                    Vector2 normalizedPosition = Vector2.Scale(vertPosition - rectTransform.rect.min, new Vector2(1f / size.x, 1f / size.y));
#endif
