...
namespace Lean.Pool
{
	...
	public class LeanGameObjectPool : MonoBehaviour, ISerializationCallbackReceiver
	{
		...
		public void PreloadOneMore()
		{
			...
			{
				// Create clone
				var parent = Strategy == StrategyType.ActivateAndDeactivate ? transform : DeactivatedChild;

				var clone = CreateClone(Vector3.zero, Quaternion.identity, Vector3.one, parent, false);
